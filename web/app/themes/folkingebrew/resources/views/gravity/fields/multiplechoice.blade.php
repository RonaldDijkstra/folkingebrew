...
@php
  ...
  // Selection mode configuration - check for custom properties or use defaults
  $selectMode = property_exists($field, 'selectMode') ? $field->selectMode :
                ($isRadio ? 'single' : 'multiple');

  // Advanced feature settings
  $exactCount = property_exists($field, 'exactCount') ? (int) $field->exactCount : 1;
  $minRange = property_exists($field, 'minRange') ? (int) $field->minRange : 0;
  $maxRange = property_exists($field, 'maxRange') ? (int) $field->maxRange : 0;
...
<div class="flex flex-col gap-1 multiple-choice-container"
  ...
  @if($inputType === 'checkbox' && $enableSelectAll && in_array($selectMode, ['multiple', 'range']))
  ...
  @if($selectMode === 'range')
    <div class="range-info mb-2 text-sm text-gray-600">
  ...
  @if($selectMode === 'exactly')
    <div class="exact-info mb-2 text-sm text-gray-600">
      {{ sprintf(__('Select exactly %d options', 'folkingebrew'), $exactCount) }}
    </div>
  @endif

  {{-- Selection Counter (for modes with limits) --}}
  @if(in_array($selectMode, ['exactly', 'range']) && $hasAdvancedFeatures)
    <div class="selection-counter mb-2 text-sm font-medium" style="display: none;">
      <span class="current-count">0</span> {{ __('selected', 'folkingebrew') }}
    </div>
  @endif
    ...
    @if($enableOther)
      @php
        ...
        if ($inputType === 'radio') {
          ...
          $otherTextName = 'input_' . $fieldId . '.' . $otherSuffix . '_text';
        }
        ...
        if (function_exists('rgpost')) {
          if ($inputType === 'radio') {
            ...
            $otherTextPost = rgpost('input_' . $fieldId . '_' . $otherSuffix . '_text');
            $otherSelected = !empty($otherPost);
            $otherTextValue = $otherTextPost ?: '';
          }
        }
      @endphp
      ...
      <div class="flex items-center gap-2 choice-item other-choice gchoice">
        <input
          ...
          @if($isRequired) aria-required="true" @endif
          @if($failed) aria-invalid="true" @endif
          class="choice-input other-choice-input @if($failed) border-red-500 @else border-gray-300 @endif"
