    ...
    class ProductCardComponent < Middleman::Extension
      helpers do 
        def product_card(opts)
          ...
          concat(
            link_to(product.url, class: "bg-white p-6") do
        ...
        def build_price(product)
          return old_price(product) + current_price(product) if product.data.old_price
          current_price(product)
        end

        def current_price(product) 
          price = number_to_currency(product.data.price, :unit => "").gsub!(/\./,",")
          content_tag(:div, "€ #{price}" , class: "font-bold whitespace-nowrap text-green-default")
        end 

        def old_price(product)
          price = number_to_currency(product.data.old_price, :unit => "").gsub!(/\./,",")
