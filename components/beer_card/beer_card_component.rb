  ...
  module BeerCard
    class BeerCardComponent < Middleman::Extension
      helpers do
        def beer_card(opts)
          beer = opts[:beer]
          image = beer.image.url(fm: :webp, h: 540, w: 540)

          concat(
            link_to("/beers/#{beer.slug}/", class: 'bg-gray-800 relative') do
              image_tag(image, alt: beer.title, class: 'block w-full', loading: 'lazy') +
              content_tag(:div,
                          class: 'opacity-0 w-full h-full bg-black absolute top-0 left-0 flex flex-col justify-center text-center p-10 hover:opacity-100') do
